...
function SecondaryButton({ children, size, rightIcon, className = '', ...props }: PropsWithChildren<Props>): JSX.Element {
  ...
  return (
    <button
      ...
      {children}
      {rightIcon && (
        <Icon type={rightIcon} width={24} height={24} className={`inline-block align-bottom ml-[2px] ${disabled && 'opacity-40 cursor-not-allowed'}`} />
      )}
    </button>
  );
}
